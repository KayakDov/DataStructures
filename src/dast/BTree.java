...
public class BTree<T extends Comparable<T>> {
    ...
    private static class BTreeNode<T extends Comparable<T>> {
         ...
         * The constructor
         *
         * @param keys The keys in this node. Node that all nodes in the tree
         * should have the same size keys array. If there are not enough keys to
         * fill the array, then the remaining elements should be null. For nodes
         * other than the root node, the array should be at least half full to
         * ensure proper BTree behavior.
         */
        public BTreeNode(T... keys) {
            if (keys == null)
         ...
         */
        public BTreeNode(int numKeys, Class cl) {
        ...
        /**
         * A lightweight class containing information about the location of a
         * key in the tree.
         */
        public class KeyLocation {
             ...
             */
            public KeyLocation(BTreeNode<T> node, int index) {
                this.node = node;
                this.index = index;
            }
        }
        ...
        /**
         * Finds the node containing the proffered key, and the index of that
         * key within the node.
         *
         * @param key The key whose node is sought.
         * @return The node, and index within that node, containing the
         * proffered key.
         */
         ...
         */
        public BTreeNode<T> insert(T... keys) {
        ...
        /**
         * Finds where this key would be inserted. If this is a leaf, inserts a
         * key into this node's keys.
         *
         ...
         * @return Same as Arrays.binarySearch, except null elements are
         * ignored.
         */
         ...
         */
        private BTreeNode(Class<?> componentType, int length) {
        ...
        private void openChildren() {
            children = (BTreeNode<T>[]) Array.newInstance(getClass(),
                    keys.length + 1);
        }
         ...
         */
        private void mergeIn(BTreeNode<T> from) {
            System.
                    arraycopy(from.keys, 0, keys, getNumKeys(), from.
                            getNumKeys());
            if (!from.isLeaf()) {
        ...
        /**
         * merges children at indicis i and i + 1, and moves key[i] into that
         * set of children.
         *
         * @param i The index of the first child to be merged.
         */
        private BTreeNode<T> mergeChild(int i) {
            BTreeNode<T> mergeChild = new BTreeNode<>(keys[0].getClass(),
                    keys.length);
         ...
         */
        private void hardDelete(int index, boolean leftGrandChild) {
            MyArray.delete(index, numKeys, keys);
            if (!isLeaf()) MyArray.delete(index + (leftGrandChild ? 0 : 1),
                        numKeys
                        + 1, children);
            numKeys--;
        }
         ...
         */
        private void insert(T key, BTreeNode<T> child, int index,
                boolean leftGrandChild) {
         ...
         */
        private void append(T key, BTreeNode<T> child) {
            keys[numKeys] = key;
        ...
        /**
         * Moves the min/max key from the right/left child up to index i / i -
         * 1, and moves the key at index i down to the ith child.
         *
         * @param childIndex The index of the child in need of a key
         * @param left Is the donator on the left (true), or the right (false).
         */
        private void rotateKey(int childIndex, boolean left) {
            BTreeNode<T> donator = children[childIndex + (left ? -1 : 1)];
            rotateKey(
                    left,
                    children[childIndex],
                    donator,
                    left ? childIndex - 1 : childIndex,
                    donator.isLeaf() ? null : donator.children[left
                    ? donator.numKeys + 1 : 0]
            );
        }
         ...
         * Rotates the key.
         *
         * @param left Is the donator on the left or the right of the key.
         * @param reciever The end receiving BTree.
         * @param donator The BTree that loses the key.
         * @param keyIndex The index of the intermediary key.
         * @param grandChGift The children of the donator.
         */
        private void rotateKey(boolean left, BTreeNode<T> reciever,
                BTreeNode<T> donator, int keyIndex, BTreeNode<T> grandChGift) {
            ...
            if (left) donator.hardDeleteLast();
            else donator.hardDelete(0, true);
        }

        /**
         * Should this node no longer be the root?
         *
         * @return True if this node should no longer be the root, false
         * otherwise.
         */
         ...
         */
        private BTreeNode<T> deleteKeyNotHere(int i, T key) {
        ...
        /**
         * Takes a node from the proffered BTree and deletes the node in that
         * tree.
         *
         * @param takeFromChild The child index from which the key is to be
         * taken.
         ...
         */
        private BTreeNode<T> delete(T... keys) {
            BTreeNode<T> root = this;
            for (T key : keys)
         ...
         * @return The root of the tree.
         */
        private BTreeNode<T> delete(T key) {
            int i = indexOf(key);
         ...
         * index.
         *
         * @param childIndex The index of the child whose grandchildren need to
         * be split.
         */
        private void splitGrandChildren(int childIndex,
                BTreeNode<T>[] grandChildren) {

            MyArray<BTreeNode<T>> grandChildKeys = MyArray.split(grandChildren,
                    children[childIndex].getNumKeys() + 1);
            children[childIndex].children = grandChildKeys.left;
            children[childIndex + 1].children = grandChildKeys.right;
        }

        /**
         * Splits this child at the given index, pulling the median element of
         * the child up into this node.
         *
        ...
        private void splitChild(int childIndex) {
            ...
            BTreeNode<T>[] grandChildren = child.children;
            ...
            MyArray.insert(childIndex,
                    ++numKeys,
                    new BTreeNode<>(childKeys.left),
                    children);

            children[childIndex + 1] = new BTreeNode<>(childKeys.right);
        ...
        public String toString() {
            ...
            Arrays.stream(keys).filter(key -> key != null).forEach(key -> sb.
                    append(
                            key).append(" "));
            sb.append("\n");
            if (!isLeaf()) {
    ...
    private static void test() {
        BTree<Integer> testTree = new BTree<>(3);
